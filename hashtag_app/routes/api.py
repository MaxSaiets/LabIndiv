...
# Безпечний імпорт Colab summarizer (може не бути налаштовано)
try:
    from ..ai_colab import get_colab_summarizer
except ImportError:
    # Якщо модуль не може бути імпортований, просто не використовуємо Colab
    def get_colab_summarizer():
        return None
    ...
    try:
        ...
        if use_ai and len(text) > 50:  # Використовуємо ШІ тільки для довгих текстів
            try:
                ...
                # Якщо Colab не працює, просто продовжуємо до Hugging Face
                print(f"Colab summarizer error (ignored): {e}")
            ...
            if not ai_summary:
                try:
                    ...
                    # Якщо і Hugging Face не працює, продовжуємо без резюме
                    print(f"Hugging Face summarizer error (ignored): {e}")
        ...
        if current_user.is_authenticated and hashtags:
            try:
                ...
                # Якщо не вдалося зберегти, продовжуємо без збереження
                print(f"Database save error (ignored): {e}")
                db.session.rollback()
